...
- Machine Learning Environment: Miniconda was utilized as the primary Python distribution to manage packages and dependencies efficiently. Jupyter Notebook served as the development environment for training, testing, comparing and visualizing multiple classification models (Decision Tree, Logistic Regression, and Random Forest). 
...
- Database Management: MongoDB Atlas was chosen as the primary cloud-hosted NoSQL database to manage applicant data, academic records, screening outcomes and administrative decisions. PyMongo was used as the driver to interface with the six operational collections.
...
The backend acts as the central coordinator. The `app.py` script initialises the Flask server, while `routes.py` manages incoming HTTP requests and triggers the evaluation modules. The database layer is implemented using PyMongo, connecting to MongoDB Atlas and organising data across six collections: `applicants`, `academic_records`, `uploaded_documents`, `screening_results`, `ml_results` and `admin_approvals`.
...
Using Miniconda and Jupyter Notebook, a simulated dataset structured around Bingham University's admission requirements was imported using the `Pandas` library. Three classifiers were evaluated using `Scikit-learn`: a Decision Tree (max_depth=6), Logistic Regression (max_iter=1000), and a Random Forest (100 estimators, max_depth=8). After comparative evaluation on the held-out test set (20% of 1,000 samples), the Random Forest classifier achieved the highest accuracy of 77.00%, outperforming the Decision Tree (74.50%) and Logistic Regression (63.50%). The trained Random Forest model was serialised using Joblib and integrated into the Flask backend (`models/` directory), allowing the system to pass the applicant's normalised data into the model to return a probabilistic suitability score.
...
- Administrative Dashboard (`admin.html`): The system aggregates the outputs from the rule-engine, the OCR module, and the Random Forest classifier into a secure management portal. 
...
- Model Evaluation: Comparative evaluation of three classifiers on the 200-sample test set produced the following results: Decision Tree — 74.50%, Logistic Regression — 63.50%, and Random Forest — 77.00%. The Random Forest classifier was selected as the production model due to its superior accuracy and robustness in handling the non-linear decision boundaries inherent in multi-class admission screening.
- Integration Testing: End-to-end testing verified that the data flowed correctly from the glassmorphism frontend, through the OCR and screening pipelines, into MongoDB Atlas, and ultimately to the protected administrative dashboard without data loss or unauthorised access.
...
5.1 Summary
This project successfully designed and implemented a Hybrid Admission Pre-Screening System tailored for Nigerian Tertiary Institutions, utilising Bingham University as a case study. Driven by the Design Science Research Methodology (DSRM), the research addressed the inefficiencies, biases, and vulnerabilities associated with manual admission screening. By implementing a START-CHECK-STOP workflow, the system integrates a deterministic rule-based engine to enforce strict institutional policies, an OCR module to detect document forgery, and a Machine Learning Random Forest classifier (selected after comparative evaluation against Decision Tree and Logistic Regression models with an accuracy of 77.00%) to rank eligible candidates. The prototype was developed using Python, Flask, MongoDB Atlas and Miniconda, culminating in a secure, decision-support platform that empowers university administrators.
...
2. Advanced Ensemble Models: Building upon the current Random Forest classifier by exploring more advanced ensemble approaches such as Gradient Boosting (XGBoost, LightGBM) or deep learning models to further improve predictive accuracy, particularly when larger, real-world admission datasets become available.
3. Scalability and Performance Optimisation: Enhancing the current MongoDB Atlas deployment with database sharding, indexing strategies and caching layers (e.g., Redis) to efficiently handle concurrent traffic from thousands of applicants during peak admission periods.
